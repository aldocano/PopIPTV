...
import com.pop_al.poptv.model.LiveStreamCategoryIdDBModel;
import com.pop_al.poptv.model.LiveStreamsDBModel;
import com.pop_al.poptv.model.pojo.PanelAvailableChannelsPojo;
import com.pop_al.poptv.model.pojo.PanelLivePojo;
import com.pop_al.poptv.model.pojo.PanelMoviePojo;
...
public class LiveStreamDBHandler extends SQLiteOpenHelper {
    ...
    public int getStreamsCount(String type) {
        try {
            Cursor cursor = getReadableDatabase().rawQuery("SELECT  COUNT(*) FROM iptv_live_streams WHERE stream_type='" + type + "'", null);
            cursor.moveToFirst();
            int count = cursor.getInt(0);
            cursor.close();
            return count;
    ...
    public ArrayList<LiveStreamsDBModel> getAllLiveStreamsArchive(String cateogryId) {
        ...
        if (cateogryId.equals(AppConst.PASSWORD_UNSET)) {
            ...
            try {
                cursor = getReadableDatabase().rawQuery("SELECT  * FROM iptv_live_streams WHERE stream_type='live' AND tv_archive='1' AND epg_channel_id IS NOT NULL AND epg_channel_id !='' ORDER BY added DESC", null);
                if (cursor.moveToFirst()) {
                    do {
        ...
        liveStreamsList = new ArrayList();
        try {
            cursor = getReadableDatabase().rawQuery("SELECT  * FROM iptv_live_streams WHERE stream_type='live' AND tv_archive='1' AND categoryID ='" + cateogryId + "' AND " + KEY_EPG_CHANNEL_ID + " IS NOT NULL AND " + KEY_EPG_CHANNEL_ID + " !='' ORDER BY " + "added" + " DESC", null);
            if (cursor.moveToFirst()) {
                do {
