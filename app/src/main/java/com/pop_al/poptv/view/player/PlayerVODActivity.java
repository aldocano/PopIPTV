...
public class PlayerVODActivity extends Activity implements OnClickListener, EventListener, VisibilityListener, OnAudioFocusChangeListener {
    ...
    private PhoneStateListener phoneStateListener = new PhoneStateListener() {
        public void onCallStateChanged(int state, String incomingNumber) {
            if (state == 1) {
                PlayerVODActivity.this.releasePlayer();
            } else if (state == 0) {
    ...
    private Uri buildURI(String stream_type, int stream_id, String container_extension) {
        ...
        try {
            builder.scheme("http").encodedAuthority(serverUrl + ":" + serverPort).appendPath(stream_type).appendPath(username).appendPath(password).appendPath(Integer.toString(stream_id) + "." + container_extension);
